...
const OrderSummary = ({cartItems, restaurant, removeFromCart}: Props) => {
    ...
    const getTotalCost = () => {
        const totalInDolar = cartItems.reduce((total, cartItem) => total + (cartItem.price / 100) * cartItem.quantity, 0)
        const totalWithDelivery = totalInDolar + (restaurant.deliveryPrice / 100)
